...
function deleteIter(arr) {
  return Array.from(new Set(arr));
}

function mostSpokenLanguages(arr, num) {
  let total = arr
    .map((el) => (el = el.languages))
    .join(",")
    .split(",")
    .sort();
  ...
  for (let index = 0; index < langs.length; index++) {
    ...
    result.push({ name: langs[index], count: counter });
  }
  return result
    .sort(function (a, b) {
...
mostPop = mostPop.sort((b, a) => a.population - b.population).slice(0, 10);
...
popButton.addEventListener("click", function () {
  label.textContent = "10 most country populated in the worled";
  container.innerHTML = "";
  for (let country of mostPop) {
    container.innerHTML += `<p> ${country.name}</p>`;
    container.innerHTML += `<div style = " width:${
      (country.population * 100 * 5) / world
    }%"> </div>`;
    container.innerHTML += `<p> ${country.population.toLocaleString(
      "en-US"
    )}</p>`;
  }
});
lanButton.addEventListener("click", function () {
  label.textContent = "10 most spoken languages in the worled";
  container.innerHTML = "";
  for (let lang of mostLan) {
    container.innerHTML += `<p> ${lang.name}</p>`;
    container.innerHTML += `<div style = "background-color: rgb(255, 171, 36); width:${
      (lang.count * 100) / 112
    }%"> </div>`;
    container.innerHTML += `<p> ${lang.count.toLocaleString(
      "en-US"
    )}</p>`;
  }
});
